...
date_time_pair_t lib::find_first_busiest_time(const cars_density_t& cars_density)
{
    ...
    std::vector<int> cars_count(cars_density.size(), 0);
    std::transform(cars_density.begin(), cars_density.end(), cars_count.begin(),
                   [count = 0](const auto& pair) mutable -> int {
                       count += pair.second;
                       return count;
                   });
    auto iter = std::max_element(cars_count.begin(), cars_count.end());

    auto busiest_time = cars_density.begin();
    std::advance(busiest_time, iter - cars_count.begin());
